...
if __name__ == '__main__':
    ...
    for cnt, noisy_file in tqdm(enumerate(noisy_files)):
        ...
        multi_channel = y.dim() > 1 and y.size(0) > 1
        
        if multi_channel:
            # Store original dimensions for multi-channel
            T_orig = y.size(1)
        ...
        if multi_channel:
            ...
            for ch in range(num_channels):
                Y_ch = torch.unsqueeze(model._forward_transform(model._stft(y[ch].cuda())), 0)
                Y_ch = pad_spec(Y_ch)
                Y_channels.append(Y_ch)
            
            # Stack channels for multi-channel processing
            Y = torch.stack(Y_channels, dim=1)  # [B, C, F, T, 2]
        else:
            # Original single-channel processing
