...
class VFModel(pl.LightningModule):
    ...
    def forward(self, x, t, y):
        # Handle multi-channel input if y has an extra dimension
        if y.dim() > 4:  # [B, C, F, T, 2] for multi-channel complex spectrograms
            batch_size, num_channels = y.shape[0], y.shape[1]
            
            # Process each channel separately and then combine
            scores = []
            for ch in range(num_channels):
                y_ch = y[:, ch]  # [B, F, T, 2]
                # Concatenate single channel y with x
                dnn_input_ch = torch.cat([x, y_ch], dim=1)
                # Get score for this channel
                score_ch = -self.dnn(dnn_input_ch, t)
                scores.append(score_ch)
            
            # Average scores from all channels
