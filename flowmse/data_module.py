...
class Specs(Dataset):
    def __init__(self, data_dir, subset, dummy, shuffle_spec, num_frames,
            ...
            stft_kwargs=None, multi_channel=False, channel_mode="average", **ignored_kwargs):
    ...
    def __getitem__(self, i):
        ...
        if self.multi_channel and y.dim() > 1 and y.size(0) > 1:
            # y is multi-channel [C, T]
            ...
            # Process each channel of noisy input
            Y_channels = []
            for ch in range(num_channels):
                Y_ch = torch.stft(y[ch], **self.stft_kwargs)
                Y_ch = self.spec_transform(Y_ch)
                Y_channels.append(Y_ch)
            
            # Stack all channels along a new dimension
            Y = torch.stack(Y_channels, dim=0)  # [C, F, T, 2] for complex
            
            # Process clean reference (single channel)
...
class SpecsDataModule(pl.LightningDataModule):
    ...
    def add_argparse_args(parser):
        ...
        parser.add_argument("--multi_channel", action="store_true", help="Enable multi-channel input processing.")
        parser.add_argument("--channel_mode", type=str, choices=("average", "concat"), default="average", help="How to handle multi-channel input: average channels or concatenate them.")
        return parser
    ...
    def __init__(
        ...
        gpu=True, normalize='noisy', transform_type="exponent", multi_channel=False,
        channel_mode="average", **kwargs
    ):
        super().__init__()
